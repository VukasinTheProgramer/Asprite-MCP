...
| `--frame-range` is **0-based**, unlike every 1-based frame index elsewhere | Passing a 1-based frame straight through exports frame N+1; the last frame always exports blank | Re-confirmed 2026-08-10 after an off-by-one shipped in `export`: `--frame-range 0,0` on a 1-frame sprite gives real pixels, `1,1` gives an all-transparent PNG. Always send `frame - 1` |
| `--frame-range` past the sprite's actual frame count | Exits 0, writes a **fully blank** PNG — no error, no warning, and the file does land so an exists() check passes | The M7 spike saw exit 0 + a file and wrongly recorded this as "clamps to frame 1". It does not clamp. Validate the frame count via the bridge before ever invoking the CLI |
